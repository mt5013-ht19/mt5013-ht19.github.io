...
topics <- c(
    ...
    "[R Programming: Programming with functions.](Class/day12.html)<br>09:15-10:15 Guest lecture: Magnus Gudmundson, Watercircles",
...
##@param lecture_number First lecture number to remove URLs from.
##
remove_url_from_topics <- function(topics, lecture_number=1) {
    stopifnot(lecture_number>0)
    
    result <- stringr::str_replace(topics[lecture_number:length(topics)], "\\([^)]*\\)", "")
    result <- stringr::str_replace(result, "\\[([^)]*)\\]", "\\1")
    
    return(c(topics[seq_len(lecture_number-1)], result))
}

##Remove URLs. Increment this for every lecture we do.
topics <- remove_url_from_topics(topics, lecture_number= 1)
```

```{r TIMEEDIT, echo = FALSE, message=FALSE, warning=FALSE, results="asis"}
cat(paste0("The schedule of the MT5013 HT2019 course is automatically fetched from [TimeEdit](",stringr::str_replace(TimeEdit_URL,"\\.json",""),")<p>"))
```
